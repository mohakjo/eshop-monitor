...
<template>
  <form @submit.prevent="onSubmit()" class="my-8">
    ...
    <p v-if="errorForm" class="w-fit rounded bg-red-500 px-2 text-red-100">
      {{ errorForm }}
    </p>
    ...
    <div>
      ...
      />
      <label for="ship-to-different-address">
        Expédier à une adresse différente
      </label>
    </div>
    <CheckoutFormShipping
      v-if="checkout.shipToDifferentAddress"
